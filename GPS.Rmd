...
<p/>
<p>
<a class='btn btn-large btn-info' href='index.html#33'>Подключение элементов питания</a>
<p/>
<p>
...
Получение координат известных географических обектов (населённых пунктов) возможно на специализированных сайтах, например [LatLong](http://www.latlong.net/)...
...
> - **Космический сегмент** представлен спутниками GPS, которые транслируют сигнал из космоса.
...
Время возврата радиоволны t пропорционально расстоянию R до препятствия, котрое может быть вычислено (в простейшем случае) по формуле:
$$
R = {{c_0 \times t} \over 2}
$$
...
> 1. **"Чистое небо"**: используйте GPS на открытом пространстве, вдали от домов и деревьев.
...
<div style='text-align: center;'>
    ...
    <font color="yellow">
        ...
        Например, имя программы может быть <i>12345678.img</i>.
    </font>
</div>
...
> - При установке карты важно иметь лишь файл \*.img. Файлы \*.typ могут и осутствовать. Это не препятствует установке карты на компьютер.
...
> - Теперь можно установить на навигатор. Запустите программу *BaseCamp* и подключите через USB порт устройство Garmin Dakota.
...
<table width="100%" cellpadding="0" cellspacing="0">
    <tr valign="top">
        ...
        <ul>
            ...
            <li>Для  отображения страницы связи со спутниками нажмите и удерживайте примерно 2 секунды значок идикатора.</li>
        </ul>    
		</td>
	</tr>
</table>
...
> - Во-вторых, независимо от того, находится навигатор в режиме приёма спутниковых сигналов 2D/3D или GPS отключён, путевую (маршрутную) можно создать с помощью страницы карты. Находясь на странице карты надо, прикоснувшись пальцем к сенсорному экрану, поместить указатель в виде булавки на объект / точку, для которых надо создать маршрутную точку. 
